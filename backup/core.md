...
## JS/jQuery规则
- **模块化**：将功能分解为独立的函数，每个函数只做一件事，并且命名清晰。
- **事件处理**：使用事件委托，避免直接在多个元素上绑定事件，尤其是动态生成的元素。
- **变量和函数命名**：使用有意义的命名，避免缩写，使用驼峰命名法。
- **注释**：在函数上方注释函数的作用，参数和返回值，复杂的逻辑需要添加行内注释。
- **错误处理**：对于可能出错的地方，添加适当的错误处理。
- **避免全局污染**：尽量使用IIFE或模块模式，将变量和函数封装在局部作用域内。
- **使用严格模式**：在JS文件开头使用'use strict'。
- **性能**：避免频繁操作DOM，可以使用文档片段（DocumentFragment）或先隐藏元素再操作最后显示。
- **链式调用**：对于jQuery，可以使用链式调用，但注意不要过长，一般不超过5个方法。
- **缓存jQuery对象**：对于重复使用的jQuery对象，应该缓存起来。

## CSS规则
- **命名规范**：使用BEM（块、元素、修饰符）或类似的命名规范，确保类名语义化。
- **避免使用ID选择器**：使用类选择器，提高可复用性。
- **避免使用!important**：除非必要，否则不要使用，优先考虑提高选择器特异性。
- **模块化**：将样式按模块组织，一个模块一个文件（如果使用构建工具）。
- **响应式设计**：使用媒体查询，确保页面在不同设备上的表现。
- **使用CSS变量**：对于重复使用的值（如颜色、字体大小），使用CSS变量，便于维护。
- **注释**：每个模块或复杂样式处添加注释。
- **性能**：避免使用通配符选择器，减少嵌套。